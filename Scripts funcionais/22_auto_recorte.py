...
PORTA = '/dev/rfcomm0'
BAUDRATE = 9600
ARQUIVO_SAIDA = 'respostas_7E1.txt'
ECU = '7E0'
...
def main():
    ...
    with open(ARQUIVO_SAIDA, 'a') as f:
        try:
            for pid in range(0x0000, 0x0030):  # 0000 até FFFF
                pid_hex = f"{pid:04X}"
            ...
            for pid in range(0x0190, 0x0240):  # 0000 até FFFF
                pid_hex = f"{pid:04X}"
                comando = f"22 {pid_hex}"
                resp = enviar_comando(ser, comando)
            ...
            for pid in range(0xf180, 0xf1ff):  # 0000 até FFFF
                pid_hex = f"{pid:04X}"
                comando = f"22 {pid_hex}"
                resp = enviar_comando(ser, comando)
